...
namespace GAZE
{
	    ...
	    void DataDrawer::Run(Scene & scene)
		{
			...
			if (scene.drawerParams.drawFaces)
			{
				...
				{
					cv::rectangle(frame, cv::Rect(scene.faces[i].first.first,
											      scene.faces[i].first.second,
			...
			if (scene.drawerParams.drawEyes)
			{
				...
				{
					cv::circle(frame,cv::Point(scene.detectedEyes[i].first,
											scene.detectedEyes[i].second),
