...
namespace GAZE
{
		...
		std::vector<Rect> RoiDetector::LocalizeEye(std::vector<Rect> faces)
		{
		    ...
		    for (int i = 0; i < faces.size(); i++)
			{
					...
					ans[i*2] = {{face.first.first  + dist1,
							     face.first.second + dist2 },
							    {face.first.first  + 2*dist1 + dist2,
                                 face.first.second + 2*dist2 }};
				    ans[i*2 + 1] = {{face.first.first + 2*dist1 + dist2,
					                face.first.second + dist2},
							        {face.first.first + 3*dist1 + 2*dist2,
									face.first.second + 2*dist2}};
			}	
			return ans;
		}
